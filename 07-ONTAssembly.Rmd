...
```{bash,eval=FALSE}
mkdir -p ~/nodule-assembly/3469-3
...
<!-- You might get a few more things corrected in a second iteration of polishing; maybe 3 at the most-->

<!-- Flye on 3469-3 takes >30 min. In the meantime, get the other sample running (which will be quicker because it doesn't have nearly as much coverage. You can also jump to the table at the end comparing read and assembly analyses. And you can introduce checkm through the video.)-->
...
```{bash,eval=FALSE}
ln -s ~/nodule-assembly/3469-3/assembly.fasta ~/nodule-assembly/assembly-3469-3.fasta .
```
...
Make sure you link it with in ~/nodule-assembly/ with the sample name.

```{bash,eval=FALSE}
ln -s ~/nodule-assembly/4956-3/assembly.fasta ~/nodule-assembly/assembly-4956-3.fasta .
```


Change into the directory with the assembly links that include the sample names, if you are not already there.
...
Let's look at the assembly metrics generated by checkM. Both samples should be in there.
