...
-x &nbsp;&nbsp; Path to the indexes (/home/data/metagenomics/centrifuge/complete_genomes/Arc_Bac_Vir_Hum_Eupath_v2)  
...
Option A:
Go to the website: https://fbreitwieser.shinyapps.io/pavian/

Option B:
If you prefer to work locally and did the Rstudio install, open up RStudio and type "pavian::runApp(port=5000)" in the console.
...
If you filter the compare table by "Escherichia coli" then sort by one of the 2 STEC samples (click once so that it sorts descending), that strain with the highest number or reads (and the highest percentage) in these 2 samples is the shiga toxin strain.

Note: The shiga strain is on the second line because the first line contains all E. coli strains.

Of course, if you knew what strain you were interested in (O26:H11) up front, you can filter the table for that directly.
